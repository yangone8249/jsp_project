...
public class BoardController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "insert":
			...
			destPage = "list"; //destPage는 에러 발생해도 페이지 이동이 가능하도록 try 이후에 설정함.
			break;
		case "list":
			log.info(">>> list 진입!");
		...
		case "update":
			...
			bsv.update(bvo2);
			request.setAttribute("updateAlert", 0);

			destPage = "list"; //list.jsp로 바로가면 list데이터를 안가져가기때문에
			// controller를 통해 list.jsp를 가야한다.
			// destPage="list" <= 이렇게 작성해야 controller의 "list"로 통할수 있다.
		...
		case "remove":
			...
			bsv.remove(Integer.parseInt(request.getParameter("num")));
			destPage = "list";
			break;
		default:
				break;
