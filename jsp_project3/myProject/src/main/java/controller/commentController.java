...
public class commentController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(pathUri.contains("/")) {//pathUri에서 / 가있다면 if문 진입
			path = pathUri.substring(0, pathUri.lastIndexOf("/")); // /path/를 출력 (마지막 /는 제거)
			pathVar = pathUri.substring(pathUri.lastIndexOf("/")+1); // path... /숫자 를 출력
		}
		log.info("path:"+path);
		...
		case "post":
			try {
				...
				while((line = br.readLine()) != null) {//null이 아니라면... (값이 남아있다면 if문 진행)
		...
		case "list":
			try {
				...
				for(int i=0; i<list.size(); i++) {
					jsonObjArr[i] = new JSONObject(); //jsonObj의 첫번째 배열을 
		...
		case "modify":
			try {
				...
				StringBuffer sb = new StringBuffer();
				String line = null;
				BufferedReader br = request.getReader(); //br에 댓글객체를 대입.
				
				while((line = br.readLine()) != null) {//null이 아니라면 while문 진입
					sb.append(line);
				}
				log.info(">>> sb : "+sb.toString());
				
				JSONParser parser = new JSONParser(); // String을 Json으로 바꿔주는 객체(JSONParser)
				JSONObject jsonObj = (JSONObject)parser.parse(sb.toString()); //sb로 모아둔걸 json객체로 변환 후 실제 Obj로 다시 변경
