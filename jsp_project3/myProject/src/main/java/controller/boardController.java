...
public class boardController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "detail":
			...
			break;
		case "modify":
			log.info(request.getParameter("image"));
			request.setAttribute("regdate", request.getParameter("regdate"));
			request.setAttribute("num", request.getParameter("num"));
			request.setAttribute("title", request.getParameter("title"));
			request.setAttribute("writer", request.getParameter("writer"));
				...
				for(FileItem item : itemList) {
					...
					case "new_file":
						if(item.getSize() > 0) { //item의 새로운파일이 등록되있으면 size가 0보다 크게된다.
							...
							//FileHandler를 호출한 후 기존파일(썸네일도)을 삭제.
		...
		case "remove":
			...
			break;
		case "removeFile":
			String imageFileName = bsv.getFileName(num);
			if(request.getParameter("image") != null) { //기존의 파일이 있으면 지우게 하도록 if문을 만든다.
				//FileHandler를 호풀한 후 기존파일(썸네일도)을 삭제.
				FileHandler fileHandler = new FileHandler();
				isOk = fileHandler.deleteFile(request.getParameter("image"), savePath);
				}
