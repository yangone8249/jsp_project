...
// 댓글 가져오기
// 컨트롤러에 가서 bno에 해당하는 댓글 리스트를 달라고 요청
async function getCommentListFromServer(bno){
	try{
		//데이터가 controller로 이동한 후 컨트롤러를 처리 하고
		// 응답 데이터를 가져와서 resp에 저장한다.
		const resp = await fetch('/cmt/list/'+bno); // /cmt/list/1
		const cmtList = await resp.json(); //댓글 객체가 리턴됨.
		return cmtList;
...
function printCommentList(bno){
	getCommentListFromServer(bno).then(result =>{
		console.log(result);
		if(result.length > 0){ //등록된 댓글이 있다면, if문 참
...
function spreadCommentList(result){//댓글 List
	...
	for(let i=0; i<result.length; i++){
		...
		html += `</div></div></div>`;	
		div.innerHTML += html; //위에 누적해놓은 html변수를 div.innerHTML사용하여 += 하기.
	}
}
...
document.addEventListener('click', (e)=>{ // 매개변수 e : 내가 클릭한 객체
