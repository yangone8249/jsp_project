...
public class ProductController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "/detail.pd":
			...
			break;
		case "/modify.pd":
			int pno1 = Integer.parseInt(request.getParameter("pno")); //String을 int로 변환하기
			p = svc.detail(pno1);
			request.setAttribute("pvo", p);
			System.out.println(">>> 상품수정페이지 성공~!!");
			destPage = "/modify.jsp";
		...
		case "/update.pd":	
			...
			String updateName = request.getParameter("pname");
			int updatePrice = Integer.parseInt(request.getParameter("price"));
			String updateMadeby = request.getParameter("madeby");
			isOk = svc.update(pno2, updateName, updatePrice, updateMadeby);
